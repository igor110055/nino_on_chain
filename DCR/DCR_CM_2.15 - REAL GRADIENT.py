...
positivo = np.ma.masked_where(0 < df['DeltaGradient'], df['mcap'])
negativo = np.ma.masked_where(0 >= df['DeltaGradient'], df['mcap'])

if df['DeltaGradient'].iloc[-1] >= 0:
    pn = 1
else:
    pn = -1
...
line1 = ax1.plot(df['date'], df['DeltaGradient'], color='cyan', label='Delta Gradient: ' + str(pn))
...
ax2 = ax1.twinx()
ax2.plot(df['date'], positivo, color='r')
ax2.plot(df['date'], negativo, color='lime')
ax2.plot(df['date'], df['realcap'], color='w')
