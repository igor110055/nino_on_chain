...
df = btcusd.merge(dcrblk, on='date', how='left').merge(btcblk, on='date', how='left').merge(dcrusd, on='date', how='left').merge(dcrmcap, on='date', how='left').merge(btcmcap, on='date', how='left').merge(dcrreal, on='date', how='left').merge(btcreal, on='date', how='left').merge(
    ltcmcap, on='date', how='left').merge(ltcreal, on='date', how='left').merge(ltcblk, on='date', how='left')
df.columns = ['date', 'btcusd', 'dcrblk', 'btcblk', 'dcrusd', 'dcrmcap', 'btcmcap', 'dcrreal', 'btcreal', 'ltcmcap', 'ltcreal', 'ltcblk']
...
line5 = ax1.plot(df['date'], df['ltcvalbyte'], label='LTC $/Byte', color='lime')
line6 = ax1.plot(df['date'], df['ltcrealbyte'], label='LTC Real $/Byte', linestyle='dashed', color='coral')
