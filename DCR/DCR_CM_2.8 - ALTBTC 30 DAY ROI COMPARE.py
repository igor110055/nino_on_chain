...
df_2 = df.pct_change(periods=1)

# Merge datasets

df_1['DCRBTC'] = df
df_1['ROI'] = df_2
df_1.columns = ['date', 'DCRBTC', 'ROI']
...
plt.plot(df_1['date'], df_1['ROI'])
plt.fill_between(df_1['date'], df_1['ROI'], color='r')
...
plt.plot(df_1['date'], df_1['DCRBTC'])
plt.title("DCRBTC")
plt.yscale('log')
plt.grid()
plt.show()
