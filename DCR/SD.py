...
fig.add_trace(
    go.Scatter(x=df['start'], y=df['PriceBTC'], name="DCRBTC Price", fill="none", line=dict(color="#000000")),
    secondary_y=False,
)

fig.add_trace(
    go.Scatter(x=df['start'], y=df['demand_chg'], name="Supply_Demand", fill="tozeroy", line=dict(color="#FF0000")),
    secondary_y=True,
)
...
# Add figure title
fig.update_layout(
    title_text="Supply / Demand for DCR Model assuming " + str(min_rew_param) + " Mining Reward Distribution"
)
...
fig.update_yaxes(title_text="DCR Surplus / Deficit", secondary_y=True)
